...
import { Globe, Play, FileUp, Shield, Layers, ShieldCheck, Search, Lock } from 'lucide-react';
import { ImportModal } from './Modals';

const Lobby: React.FC = () => {
  const { setSession, addDebugLog } = useApp();
  ...
  const handleStart = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    addDebugLog('private_session_init', { url });
    
    setTimeout(() => {
      setSession(prev => ({
        ...prev,
        id: Math.random().toString(36).substr(2, 9),
        initialUrl: url.startsWith('http') ? url : `https://${url}`,
        status: 'recording',
        connected: true
      }));
      setIsLoading(false);
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full p-6 md:p-12 relative">
      <div className="max-w-5xl w-full flex flex-col items-center space-y-16">
        ...
        <div className="text-center space-y-6">
          ...
          <p className="text-xl text-[#6B5E55] max-w-xl mx-auto leading-relaxed font-medium">
            Turn your browser interactions into structured guides <br/>
            <span className="text-[#2D241E]">without ever leaving your machine.</span>
          </p>
        </div>
        ...
        >
          <div className="relative flex items-center bg-white/80 backdrop-blur-2xl border border-white rounded-[40px] p-2.5 shadow-[0_20px_50px_rgba(45,36,30,0.08)] transition-all duration-500 hover:shadow-[0_25px_60px_rgba(45,36,30,0.12)]">
            ...
            <input
              ...
              className="w-full bg-transparent border-none focus:ring-0 text-lg px-4 py-3 placeholder:text-[#BBAFA7] font-semibold text-[#2D241E]"
            />
            <button
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 w-full">
          <BentoItem 
            ...
            title="Local Engine"
            desc="All guide processing happens in your browser's memory."
            bgColor="bg-[#FAD7BD]/30"
          />
          <BentoItem 
            icon={<Lock className="w-6 h-6 text-[#2D241E]" />}
            title="Zero-Storage"
            desc="No database, no cloud accounts, and no tracking. Data stays in RAM."
            bgColor="bg-white"
            isFeatured
          />
          <BentoItem 
            ...
            title="Safe Export"
            desc="Encryption keys are generated locally and never shared."
        ...
        <button 
          ...
          className="flex items-center space-x-2 text-[#6B5E55] hover:text-[#2D241E] font-bold text-sm tracking-wide transition-all uppercase active:scale-95"
        >
      ...
      <div className="absolute bottom-10 text-[#BBAFA7] text-[10px] font-bold uppercase tracking-[0.2em]">
        Privacy-Centric Engineering • Stateless Architecture
      </div>
...
const BentoItem: React.FC<{ icon: React.ReactNode; title: string; desc: string; bgColor: string; isFeatured?: boolean }> = ({ icon, title, desc, bgColor, isFeatured }) => (
  <div className={`p-10 rounded-[48px] ${bgColor} border ${isFeatured ? 'border-[#2D241E]/5 shadow-xl' : 'border-white/60'} transition-all duration-500 hover:-translate-y-2 group overflow-hidden relative`}>
