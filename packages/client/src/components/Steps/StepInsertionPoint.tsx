...
export function StepInsertionPoint({ index, alwaysExpanded = false }: StepInsertionPointProps) {
  ...
  return (
    <button
      ...
      style={{ 
        height: alwaysExpanded ? '72px' : isHovered ? '64px' : '32px',
        margin: alwaysExpanded ? '8px 0 12px' : '4px 0',
        width: '100%'
      }}
    ...
    >
      <div className={`
        ...
        ${isExpanded ? 'bg-[#E67E22] scale-x-100 opacity-100' : 'bg-[#E67E22]/10 scale-x-[0.98] opacity-50'}
      `} />
      ...
      <div
        ...
        `}
      >
        <Plus size={isExpanded ? 18 : 12} strokeWidth={3} className="transition-all duration-500" />
      </div>
      ...
      `}>
        {alwaysExpanded ? 'Add Step at End' : 'Insert Step'}
      </div>
